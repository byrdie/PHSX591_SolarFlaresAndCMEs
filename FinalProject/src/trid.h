...
void trid2_warp(REAL &am, REAL &cm, REAL &dm,
  ...
  b   = 1.0f - ap*cm - cp*__shfl_down(am,s);
  b   = __rcp(b);
  // for some odd reason, the next line is best there not higher
  dp  =  dp  - ap*dm - cp*__shfl_down(dm,s);
  dp  =   b*dp;
  ap  = - b*ap*am;
...
void trid_warp_setup(REAL *a,  REAL *b,  REAL *c, REAL *atmp,
  ...
  bb0 = __rcp(1.0f - atmp[7]*c0 - c[7]*__shfl_down(a0, z));
  ap[0] = - atmp[7]*a0 * bb0;
  cp[0] = - c[7]*__shfl_down(c0, z) * bb0;
...
void trid_warp_solve(REAL *a,  REAL *b,  REAL *c, REAL *atmp,
  ...
  f[7] = (f[7] - atmp[7]*f[0] - c[7]*__shfl_down(f[0], z)) * bb0;
